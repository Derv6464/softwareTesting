...
def onSubmit():
    ...
    if formChecks[0]:
        # room:room object, date:datetime object, numOfPeople:int, length:int, age:int, name:str, phone:str
...
def onTimeSubmit():
    ...
    if formChecks[0]:
        ...
        flash(formChecks[1])
        times = []
        times = c.getAvabileTimes(
            c.booking[1], c.booking[0], c.booking[3], c.getBookings()
        )
        return render_template("selectTime.html", data=times)
